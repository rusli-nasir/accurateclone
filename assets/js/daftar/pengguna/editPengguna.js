...
function formCheck() {
  ...
  $.ajax({
    url: base_url + "Daftar/Pengguna/checkUsernameUniqueWhenEdit", // URL tujuan
    type: "POST", // Tentukan type nya POST atau GET
    data: 'username_sesudah=' + input_username + '&username_sebelum=' + username_sebelum, // Ambil semua data yang ada didalam tag form
    dataType: "json",
...
$(document).ready(function () {
  username_sebelum = $('#input_username').val();
  $('#btn-change-password').click(function () {
    $('#change-password-div').hide();
    $('#input_password').val('');
    $('#input_passconf').val('');
    $('#input-group-password').show();
    $('#input-group-passconf').show();
  })
  $('#input_username').change(function () {
    $.ajax({
      url: base_url + "Daftar/Pengguna/checkUsernameUniqueWhenEdit", // URL tujuan
      type: "POST", // Tentukan type nya POST atau GET
      data: 'username_sesudah=' + $('#input_username').val() + '&username_sebelum=' + username_sebelum, // Ambil semua data yang ada didalam tag form
      dataType: "json",
      beforeSend: function (e) {
        if (e && e.overrideMimeType) {
          e.overrideMimeType("application/jsoncharset=UTF-8")
  ...
  $('#form-tambah-pengguna').submit(function (event) {
    ...
    if (formCheck()) {
      $.ajax({
        url: base_url + "Daftar/Pengguna/editPengguna/" + $('#input_username').val(), // URL tujuan
        type: "POST", // Tentukan type nya POST atau GET
